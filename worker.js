...
const ADAPTERS = {
  ...
  */
  accounting: {
    ...
    async status(env, h) {
      ...
      const real = conns.find((c) => !/demo company/i.test(c.tenantName || '')) || conns[0];
      return {
        connected: true,
        org: real.tenantName || null,
        sandbox: /demo company/i.test(real.tenantName || ''),
        lastSync: null
      };
    },
    async fetchRange(env, h, q) {
      const { tenantId } = await xeroTenant(h);
      const url = 'https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss?fromDate=' + q.from + '&toDate=' + q.to;
      const report = await h.fetchJson(url, { headers: { 'Xero-Tenant-Id': tenantId, 'Accept': 'application/json' } });
      const cols = walkXeroPL(report);
      const col = cols[0] || { revenue: null, cogs: null, wagesSuper: null, overheads: null };
      return { revenue: col.revenue, cogs: col.cogs, wagesSuper: col.wagesSuper, overheads: col.overheads };
    },
    async fetchMonthly(env, h, q) {
      ...
      for (let i = 0; i < months.length; i += 12) {
        ...
        const url = 'https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss?toDate=' + toDate + '&periods=' + chunk.length + '&timeframe=MONTH';
        const report = await h.fetchJson(url, { headers: { 'Xero-Tenant-Id': tenantId, 'Accept': 'application/json' } });
        for (const col of walkXeroPL(report)) {
          const ym = xeroLabelToMonth(col.label);
          if (ym) byMonth[ym] = col;
        }
      }
      const revenue = [], cogs = [], wagesSuper = [], overheads = [];
      for (const mo of months) {
        const c = byMonth[mo];
        revenue.push(c ? c.revenue : null);
        cogs.push(c ? c.cogs : null);
        wagesSuper.push(c ? c.wagesSuper : null);
        overheads.push(c ? c.overheads : null);
      }
  ...
  */
  pos: {
    /* Lightspeed O-Series: owner's plan doesn't include scheduled report
       export or live API access (would need a paid API Plan via her account
       manager - see capability-matrix.md). Fallback ladder rung 4: owner
       downloads the Sales Summary report from Lightspeed's front end and
       drops it on the dashboard's Connections screen upload panel (weekly
       rhythm agreed with the owner). mode:'export' + parseExport below wires
       that rung. Report columns confirmed from the owner's own export
       (2026-07-02): saledateid, saledate (YYYY-MM-DD), salecount,
       saleamount, saleexamount, saletaxamount, averageamount. `salecount` is
       the completed-transaction count per day - exactly kpi-spec.md metric 2
       (never a dollar figure from the POS, so saleamount/saleexamount are
       ignored here). Single combined report across all outlets, matching the
       owner's one Lightspeed master account. */
    configured: true,
    auth: null,
    mode: 'export',
    oauth: {},
    async status(env, h) {
      return { connected: true, org: 'Lightspeed (uploaded Sales Summary reports)', sandbox: false, lastSync: null };
    },
    async fetchRange(env, h, q) {
    ...
    async fetchMonthly(env, h, q) {
      ...
      return { months: r.months, count: r.byMonth.map((m) => (m ? m.count : null)) };
    },
    async parseExport(env, h, raw) {
      ...
      const countIdx = header.indexOf('salecount');
      if (dateIdx < 0 || countIdx < 0) return [];
      const out = [];
      for (let i = 1; i < rows.length; i++) {
        const r = rows[i];
        if (!r || r.length <= Math.max(dateIdx, countIdx)) continue;
        const date = (r[dateIdx] || '').trim();
        const count = parseInt(r[countIdx], 10);
        if (/^\d{4}-\d{2}-\d{2}$/.test(date) && isFinite(count)) out.push({ date, count });
      }
      return out;
    }
  },
...
/* Minimal quoted-field-aware CSV parser (no external deps). Handles
   "quoted,fields", embedded commas inside quotes, "" as an escaped quote,
   and \r\n or \n line endings. Returns an array of row arrays of strings. */
...
/* ---------------- Xero P&L parsing helpers (accounting adapter) ----------
   kpi-spec.md / capability-matrix.md: Revenue = Income section (excl. Other
   Income); Cost of goods = Cost of Sales section; wages+super = the owner's
   CONFIRMED account list inside Operating Expenses; Overheads = Operating
   Expenses total minus those wage/super lines.
   CONFIRMED WITH OWNER (Caroline, from her chart of accounts export, and by
   reconciliation against her May 2026 P&L) - not a keyword guess:
     Wages/super accounts: Admin Fee - P/Ship, Directors Fees, Directors
     Wages, Superannuation, Shop Wages (+ the four per-outlet Shop Wages
     accounts), Bakers Wages, Box Packer, Maternity Leave, Delivery Driver -
     Wages, Delivery Driver - Contractors, PP Contractors Wages.
     Revenue exclusion: Parental Leave Payments (a Revenue-type account, but
     the owner wants it left out of Revenue AND out of the wage calculation
     entirely - it is not netted against wages).
   If new wage/revenue accounts are added later in Xero, re-confirm the list
   with the owner (kpi-spec.md, metric 5) rather than silently guessing. */
const WAGE_ACCOUNTS = new Set([
  'admin fee - p/ship', 'directors fees', 'directors wages', 'superannuation',
  'shop wages', 'shop wages - adaminaby', 'shop wages - jindabyne',
  'shop wages - nimmitabel', 'shop wages - cooma', 'bakers wages',
  'box packer', 'maternity leave', 'pp contractors wages',
  'delivery driver - wages'
]);
const REVENUE_EXCLUDE_ACCOUNTS = new Set(['parental leave payments']);
/* CONFIRMED against the owner's May 2026 P&L (Xero's live standard report
   groups Wages, Distribution costs and Other Expenses all inside the one
   "Operating Expenses" section - unlike the owner's saved custom Tracking
   report, which shows them as separate sections). These lines must come out
   of Overheads to hit the owner's confirmed $55,471.89:
     - Delivery Driver - Wages: owner's own management report carries this
       under "Distribution costs" (excluded from her Wages subtotal there),
       but the owner asked for it counted as a wage cost on the dashboard -
       moved into WAGE_ACCOUNTS above (2026-07-02).
     - Delivery Driver - Contractors: contractor cost, not yet confirmed as
       wages - stays out of both Wages and Overheads for now (ask the owner
       if/when it carries a balance).
     - Rent - Victoria Street Cooma Res / Rent - Bombala St, Nimmitabel Res:
       residential rent, sits under "Other Expenses" in the owner's report,
       excluded from Overheads.
   "Rent - Lett St, Adaminaby Res" is also residential rent, tagged "Other
   Expenses" in the owner's chart of accounts - owner confirmed 2026-07-02 to
   exclude it from Overheads too, for consistency with the other two
   residential rent accounts (even though it sat inside her May Operating
   Expenses total in the reconciliation check). */
const OVERHEADS_EXCLUDE_ACCOUNTS = new Set([
  'delivery driver - contractors',
  'rent - victoria street cooma res', 'rent - bombala st, nimmitabel res',
  'rent - lett st, adaminaby res'
]);
const MONTH_ABBR = { jan: 1, feb: 2, mar: 3, apr: 4, may: 5, jun: 6, jul: 7, aug: 8, sep: 9, oct: 10, nov: 11, dec: 12 };

function xeroLabelToMonth(label) {
...
function numCell(cell) {
  const n = parseFloat((cell && cell.Value) || '0');
  return isFinite(n) ? n : 0;
}
function collectRows(rows, out) {
  for (const r of rows || []) {
    if (r.RowType === 'Row') out.push(r);
    else if (r.RowType === 'Section' && r.Rows) collectRows(r.Rows, out);
  }
}
function findSection(rows, titleTest) {
  for (const r of rows || []) {
    if (r.RowType === 'Section' && titleTest(r.Title || '')) return r;
  }
  return null;
}
function sectionTotals(section, numCols) {
  let summary = null;
  for (const r of section.Rows || []) {
    if (r.RowType === 'SummaryRow') summary = r;
  }
  const totals = new Array(numCols).fill(0);
  if (summary) {
    for (let i = 0; i < numCols; i++) totals[i] = numCell(summary.Cells[i + 1]);
    return totals;
  }
  const rows = [];
  collectRows(section.Rows, rows);
  for (const r of rows) {
    for (let i = 0; i < numCols; i++) totals[i] += numCell(r.Cells[i + 1]);
  }
  return totals;
}
/* Sum of individual line rows in a section whose account name is in `names`
   (lowercased, trimmed exact match) - used both to pull the confirmed wage/
   super lines out of Operating Expenses, and to strip a specifically
   excluded account (e.g. Parental Leave Payments) out of a section's total
   even when that total came from Xero's own SummaryRow. */
function sectionNamedAmount(section, numCols, names) {
  const totals = new Array(numCols).fill(0);
  const rows = [];
  collectRows(section.Rows, rows);
  for (const r of rows) {
    const label = ((r.Cells[0] && r.Cells[0].Value) || '').trim().toLowerCase();
    if (names.has(label)) {
      for (let i = 0; i < numCols; i++) totals[i] += numCell(r.Cells[i + 1]);
    }
  }
...
/* Returns one entry per report column (period): { label, revenue, cogs, wagesSuper, overheads }. */
function walkXeroPL(report) {
  const rep = report && report.Reports && report.Reports[0];
  if (!rep) return [];
  const rows = rep.Rows || [];
  const header = rows.find((r) => r.RowType === 'Header');
  const numCols = header ? header.Cells.length - 1 : 1;

  const incomeSection = findSection(rows, (t) => /income|revenue/i.test(t) && !/other income/i.test(t));
  ...
  /* Some orgs' live report layout DOES split Wages into its own section
     (rather than burying wage lines inside Operating Expenses) - prefer
     that section's own total when Xero gives us one; otherwise fall back
     to matching the owner's confirmed wage account list inside Operating
     Expenses. Either path is then reconciled to the owner's confirmed
     figures via OVERHEADS_EXCLUDE_ACCOUNTS. */
  const wagesSection = findSection(rows, (t) => /^wages$/i.test(t.trim()));

  let revenue = incomeSection ? sectionTotals(incomeSection, numCols) : new Array(numCols).fill(null);
  if (incomeSection && revenue[0] !== null) {
    const excluded = sectionNamedAmount(incomeSection, numCols, REVENUE_EXCLUDE_ACCOUNTS);
    revenue = revenue.map((v, i) => (v === null ? null : v - excluded[i]));
  }
  const cogs = cosSection ? sectionTotals(cosSection, numCols) : new Array(numCols).fill(null);

  let wagesSuper = new Array(numCols).fill(0);
  let overheadsTotal = new Array(numCols).fill(null);
  if (opexSection) {
    overheadsTotal = sectionTotals(opexSection, numCols);
    wagesSuper = wagesSection
      ? sectionTotals(wagesSection, numCols)
      : sectionNamedAmount(opexSection, numCols, WAGE_ACCOUNTS);
    const overheadsExcluded = sectionNamedAmount(opexSection, numCols, OVERHEADS_EXCLUDE_ACCOUNTS);
    overheadsTotal = overheadsTotal.map((v, i) => v - overheadsExcluded[i]);
  }

  const out = [];
  for (let i = 0; i < numCols; i++) {
    out.push({
      label: header && header.Cells[i + 1] ? header.Cells[i + 1].Value : null,
      revenue: revenue[i] === null ? null : revenue[i],
      cogs: cogs[i] === null ? null : cogs[i],
      wagesSuper: wagesSuper[i],
      overheads: overheadsTotal[i] === null ? null : (overheadsTotal[i] - wagesSuper[i])
    });
  }
  return out;
}
async function xeroTenant(h) {
  const conns = await h.fetchJson('https://api.xero.com/connections', {});
  if (!conns || !conns.length) { const e = new Error('no tenant'); e.status = 401; throw e; }
  const real = conns.find((c) => !/demo company/i.test(c.tenantName || '')) || conns[0];
  return { tenantId: real.tenantId, tenantName: real.tenantName };
}
